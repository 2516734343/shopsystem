...
public class ShoppingCarAction {
    ...
    //插入一条购物车（用户点击了加入购物车） ok
    @RequestMapping(value = "/insert",method = RequestMethod.POST)
    public int insert(@RequestBody ShoppingCarInfo shoppingCarInfo ){
    ...
    public int DeleteOneGood(@RequestBody ShoppingCarInfo shoppingCarInfo ){
        //goodid user_id
        return shoppingCarInfoService.DeleteOneGood(shoppingCarInfo);
    }

    //用户从购物车下单（删除该用户的购物车的所有商品）ok
    @RequestMapping(value = "/UserBuy",method = RequestMethod.GET)
    ...
    public int updateGoodsNumber(@RequestBody ShoppingCarInfo shoppingCarInfo){
        //user_id,pro_id,goodsnumber
